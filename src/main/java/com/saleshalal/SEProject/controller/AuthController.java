...
public class AuthController {
    ...
    @PostMapping("/login")
    public String testLogin(@RequestParam("email") String email,
                    @RequestParam("password") String password, Model model) {
        logger.info("Testing login for user: {}", email);

        try {
            if (userService.validateUser(email, password) == 1) {
                return "redirect:/customer-dashboard"; // Simplified for testing
            }
            model.addAttribute("error", "Invalid email or password");
            return "login"; // display login page
        } catch (Exception e) {
            logger.error("Error occurred while logging in: {}", e.getMessage(), e);
            model.addAttribute("error", e.getMessage());
            return "login"; // display login page
